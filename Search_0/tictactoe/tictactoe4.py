...
def result(board, action):
    ...
    try:
        ...
        print("This is not the correct type for an action")
        raise
...
def terminal(board):
    ...
    """
    board2 = [i for n in board for i in n]
    if board2.count(EMPTY) == 0:
        return True
...
def utility(board):
    ...
    """
    if winner(board) == X:
        #print("UTILITY RETURNS 1")
        return 1
    elif winner(board) == O:
        #print("UTILITY RETURNS -1")
        return -1
    else:
        #print("UTILITY RETURNS 0")
        return 0
...
def MAX_VALUE(board):
    ...
    #print("Min Actions:", actions(board))
    for action in actions(board):
        #print("action max", result(board, action))
...
def MIN_VALUE(board):
    ...
    #print("Max Actions:", actions(board))
    for action in actions(board):
        #print("action min", result(board, action))
        #if isinstance(action, int):
        #    continue
